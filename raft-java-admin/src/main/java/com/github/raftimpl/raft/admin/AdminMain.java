...
public class AdminMain {
    // Static JsonFormat object for JSON serialization
    ...
    public static void main(String[] args) {
        ...
        // servers format: "10.1.1.1:8010:1,10.2.2.2:8011:2,10.3.3.3.3:8012:3"
        String servers = args[0];
        String cmd = args[1];
        String subCmd = args[2];

        // Validate the 'cmd' and 'subCmd' arguments
        Validate.isTrue(cmd.equals("conf"));
        Validate.isTrue(subCmd.equals("get") || subCmd.equals("add") || subCmd.equals("del"));

        // Create a client proxy with the server addresses
        ...
        if (subCmd.equals("get")) {
            ...
            // Parse the server addresses for adding peers
            ...
            // Parse the server addresses for removing peers
            List<RaftProto.Server> serverList = parseServers(args[3]);
        ...
        // Stop the client service proxy
        ((RaftClientServiceProxy) client).stop();
    }

    // Parse server addresses from a string format to a list of RaftProto.Server objects
