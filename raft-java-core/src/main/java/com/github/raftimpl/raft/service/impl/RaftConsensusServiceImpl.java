...
//                    null, raftNode.getRaftLog().getFirstLogIndex());
    ...
    private void advanceCommitIndex(RaftProto.AppendEntriesRequest request) {
        ...
        // In Follower-Read (Read Index) mode, the Follower will enter a waiting state, waiting for the log to be applied to the replicated state machine. Therefore, the waiting threads need to be awakened
